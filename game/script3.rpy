...
<<<<<<< HEAD
    ...
    Скати[past_end] бы я по учебе? Дружил[verb_end] бы с «не самой хорошей компанией?» Впрочем, какая разница какими их считают? Они единственные все еще общаются со мной и зовут куда-то.
...
<<<<<<< HEAD
    ...
    "???" "..."
    aik calm "Нет, послушай! Я не считаю это чем-то плохим, но я переживаю за тебя! И родители тоже. Может, они и избирают немного жесткие методы, но это и правда ради твоего блага…"
    "???" "...!"
    aik normal "Ты собираешься сегодня в то место, о котором говорила?"
    "???" "..."
    ...
    menu:
        ...
        "Поддержать Наоми":
            ...
            $ friendshp_gh += 1
            $ renpy.notify("Отношения с Персонажем Призрак улучшились.")
            gg "Я рад(а), что смог(ла) познакомиться с тем самым таинственным художником"
